...
export default function PageEditor({ page, onUpdate, onBack, user }) {
  ...
  const [noteViewMode, setNoteViewMode] = useState("view");
  ...
  return (
    <div style={{ maxWidth: 860, margin: "0 auto", width: "100%", padding: "48px 60px 80px" }}>
      ...
      {!editing && (
        <div style={{ display: "flex", gap: 12, marginBottom: 32, justifyContent: "flex-start", borderBottom: `1px solid ${t.border}`, paddingBottom: 16 }}>
          <button
            onClick={() => {
              setNoteViewMode("view");
              setEditing(false);
            ...
            style={{
              ...
              fontWeight: 600,
              cursor: "pointer",
              background: noteViewMode === "view" ? t.accent : "transparent",
              border: `1px solid ${noteViewMode === "view" ? t.accent : t.border}`,
              color: noteViewMode === "view" ? "#fff" : t.muted,
              transition: "all 0.2s",
              outline: "none",
            }}
            onMouseEnter={(e) => {
        ...
        <>
          ...
          <div style={{ display: "flex", gap: 12, marginTop: 24, justifyContent: "flex-end" }}>
            <button
              ...
              style={{ fontSize: 14, padding: "8px 24px", borderRadius: 8, fontWeight: 500, cursor: "pointer", background: "transparent", border: `1px solid ${t.border}`, color: t.muted, transition: "all 0.2s", outline: "none" }}
              onMouseEnter={(e) => {
                e.currentTarget.style.borderColor = t.accent;
                e.currentTarget.style.color = t.accent;
                e.currentTarget.style.background = t.accentSoft;
            ...
            <button
              onClick={handleSave}
              disabled={!hasChanges}
              style={{ fontSize: 14, padding: "8px 24px", borderRadius: 8, fontWeight: 500, cursor: hasChanges ? "pointer" : "not-allowed", background: hasChanges ? t.accent : t.muted, border: "none", color: "#fff", opacity: hasChanges ? 1 : 0.5, transition: "all 0.2s", outline: "none" }}
              onMouseEnter={(e) => {
                if (hasChanges) {
                  e.currentTarget.style.background = t.accentDark || "#4a6fd8";
                }
